...
public class BTreeMap<K, V> extends AbstractMap<K, V>
		implements ConcurrentNavigableMap<K, V>, Bind.MapWithModificationListener<K, V>, Closeable, Serializable {

	public final long rootRecidRef;

	public final BTreeKeySerializer keySerializer;

	public final Serializer<V> valueSerializer;

	public final Serializer valueNodeSerializer;

	public final LongConcurrentHashMap<Thread> nodeLocks = new LongConcurrentHashMap<Thread>();

	public final int maxNodeSize;
	...
	public final List<Long> leftEdges;
	...
	public final boolean closeEngine;
	...
	public static SortedMap<String, Object> preinitCatalog(DB db) {
		...
		return new BTreeMap<String, Object>(db.engine, false, Engine.RECID_NAME_CATALOG, 32, false, 0,
				BTreeKeySerializer.STRING, valser, 0);
	}
	...
	public static final Serializer<ValRef> VALREF_SERIALIZER = new Serializer.EightByteSerializer<ValRef>() {
		...
		@Override
		public ValRef unpack(long l) {
			return new ValRef(l);
		}

		@Override
		public long pack(ValRef l) {
			return l.recid;
		}
	...
	public static final Serializer<Boolean> BOOLEAN_PACKED = new Serializer.BooleanSer() {
	...
	public abstract static class BNode {
		...
		public BNode(Object keys, boolean leftEdge, boolean rightEdge, boolean tooLarge) {
			...
			this.flags = (byte) ((leftEdge ? LEFT_MASK : 0) | (rightEdge ? RIGHT_MASK : 0)
				...
				for (int i = 1; i <= end; i++) {
					if (keyser.compare(keys, i - 1, i) >= 0)
						throw new DBException.DataCorruption(
								"keys are not sorted: " + Arrays.toString(keyser.keysToArray(keys)));
				}
			}
			if (!isRightEdge() && keylen > 2) {
				if (keyser.compare(keys, keylen - 2, keylen - 1) > 0) {
					throw new DBException.DataCorruption(
							"Last key is not sorted: " + Arrays.toString(keyser.keysToArray(keys)));
				}
			}
		}

		public abstract BNode copyAddKey(BTreeKeySerializer keyser, Serializer valser, int pos, Object newKey,
	...
	public final static class DirNode extends BNode {
		...
		public long child(int pos) {
			Object c = child;
			return c instanceof int[] ? ((int[]) c)[pos] : ((long[]) c)[pos];
		}

		@Override
		public int childArrayLength() {
			return child instanceof int[] ? ((int[]) child).length : ((long[]) child).length;
		}
		...
		@Override
		public String toString() {
			return "Dir(" + leftEdgeInc() + "-" + rightEdgeInc() + "-K" + Fun.toString(keys) + ", C"
					+ Fun.toString(child) + ")";
		}
		...
		@Override
		public DirNode copyAddKey(BTreeKeySerializer keyser, Serializer valser, int pos, Object newKey, long newChild,
				Object newValue) {
	...
	public final static class LeafNode extends BNode {
		...
		@Override
		public String toString() {
			String valsStr = Fun.toString(vals);

			return "Leaf(" + leftEdgeInc() + "-" + rightEdgeInc() + "-" + "K" + Fun.toString(keys) + ", V" + valsStr
					+ ", L=" + next + ")";
		}
		...
		@Override
		public LeafNode copyAddKey(BTreeKeySerializer keyser, Serializer valser, int pos, Object newKey, long newChild,
				Object newValue) {
			Object keys2 = keyser.putKey(keys, pos - leftEdgeInc(), newKey);
		...
		public LeafNode copyClear(BTreeKeySerializer keyser, Serializer valser) {
			...
			return new LeafNode(keyser.arrayToKeys(keys2), isLeftEdge(), isRightEdge(), false, valser.valueArrayEmpty(),
					next);
		}
	}

	public final Serializer<BNode> nodeSerializer;
	...
	public static final class NodeSerializer<A, B> extends Serializer<BNode> {
		...
		public NodeSerializer(boolean valsOutsideNodes, BTreeKeySerializer keySerializer, Serializer valueSerializer,
			...
			this.valueSerializer = (Serializer) (hasValues ? (valsOutsideNodes ? VALREF_SERIALIZER : valueSerializer)
		...
		public void serialize(DataOutput out, BNode value) throws IOException {
			...
			final int header = (isLeaf ? LEAF_MASK : 0) | (value.isLeftEdge() ? LEFT_MASK : 0)
					| (value.isRightEdge() ? RIGHT_MASK : 0) | value.keysLen(keySerializer);
		...
		private BNode deserializeDir(final DataIO.DataInputInternal in, final int size, final int left, final int right)
				throws IOException {
			Object child;
			...
			final Object keys = keysize == 0 ? keySerializer.emptyKeys() : keySerializer.deserialize(in, keysize);
	...
	public BTreeMap(Engine engine, boolean closeEngine, long rootRecidRef, int maxNodeSize, boolean valsOutsideNodes,
			long counterRecid, BTreeKeySerializer keySerializer, final Serializer<V> valueSerializer,
			int numberOfNodeMetas) {
		...
		this.nodeSerializer = new NodeSerializer(valsOutsideNodes, keySerializer, valueNodeSerializer,
				numberOfNodeMetas);
	...
	static public long createRootRef(Engine engine, BTreeKeySerializer keySer, Serializer valueSer,
			boolean valuesOutsideNodes, int numberOfNodeMetas) {
	...
	public Object get(Object key, boolean expandValue) {
		if (key == null)
			throw new NullPointerException();
		K v = (K) key;
	...
	public final long nextDir(DirNode d, Object key) {
	...
	public V put2(final K key, final V value2, final boolean putOnlyIfAbsent) {
		K v = key;
			...
			while (true) {
				...
				if (A.keysLen(keySerializer) - (A.isLeaf() ? 1 : 0) < maxNodeSize) {
					...
					if ((current != rootRecid)) { 
						...
						Object rootChild = (current < Integer.MAX_VALUE && q < Integer.MAX_VALUE)
								? new int[] { (int) current, (int) q, 0 } : new long[] { current, q, 0 };

						BNode R = new DirNode(keySerializer.arrayToKeys(new Object[] { A.highKey(keySerializer) }),
								true, true, false, rootChild);
						unlock(nodeLocks, current);
	...
	public static class BTreeDescendingIterator {
		...
		private void pointToStart() {
			...
			for (;;) {
				...
				if (next == 0) {
					...
					next = children instanceof int[] ? ((int[]) children)[((int[]) children).length - 2]
		...
		public void advance() {
			...
			if (currentPos == 0) {
				...
				Fun.Pair<Integer, BNode> prevPair = nextKey == null ? null : m.findSmallerNode(nextKey, false);
				if (prevPair == null) {
					currentLeaf = null;
					currentPos = -1;
					return;
			...
			for (;;) {
				...
				if (pos > 0 && pos != A.keysLen(keySerializer) - 1) {
					...
					if (putNewValue == null || !valsOutsideNodes) { 
						A = putNewValue != null ? ((LeafNode) A).copyChangeValue(valueNodeSerializer, pos, putNewValue)
	...
	public Entry<K, V> makeEntry(Object key, Object value) {
	...
	public Entry<K, V> findSmaller(K key, boolean inclusive) {
		if (key == null)
			throw new NullPointerException();
		final long rootRecid = engine.get(rootRecidRef, Serializer.RECID);
		BNode n = engine.get(rootRecid, nodeSerializer);
	...
	private Entry<K, V> findSmallerRecur(BNode n, K key, boolean inclusive) {
		...
		for (int i = start; i >= end; i--) {
			...
			if (comp < res) {
				if (leaf) {
					return key2 == null ? null : makeEntry(key2, valExpand(n.val(i - 1, valueNodeSerializer)));
				} else {
					...
					if (n2.isLeaf()) {
						if (n2.keysLen(keySerializer) > 2 && keySerializer.comparator()
								.compare(n2.key(keySerializer, 1), key) >= (inclusive ? 1 : 0)) {
							continue;
						}
					}
	...
	public Fun.Pair<Integer, BNode> findSmallerNode(K key, boolean inclusive) {
		if (key == null)
			throw new NullPointerException();
		final long rootRecid = engine.get(rootRecidRef, Serializer.RECID);
		BNode n = engine.get(rootRecid, nodeSerializer);
	...
	public Fun.Pair<Integer, BNode> findSmallerNodeRecur(BNode n, K key, boolean inclusive) {
		final boolean leaf = n.isLeaf();
		final int start = leaf ? n.keysLen(keySerializer) - 2 : n.keysLen(keySerializer) - 1;
		final int end = leaf ? 1 : 0;
		final int res = inclusive && leaf ? 1 : 0;
		for (int i = start; i >= end; i--) {
			...
			if (comp < res) {
				if (leaf) {
					return key2 == null ? null : new Fun.Pair(i, n);
				} else {
					final long recid = n.child(i);
					if (recid == 0)
						continue;
	...
	public Entry<K, V> findLarger(final K key, boolean inclusive) {
		if (key == null)
			return null;
		...
		while (true) {
			...
			for (int i = 1; i < leaf.keysLen(keySerializer) - 1; i++) {
				...
				if (leaf.key(keySerializer, i) == null)
					continue;
	...
	public Fun.Pair<Integer, LeafNode> findLargerNode(final K key, boolean inclusive) {
		if (key == null)
			return null;
	...
	public static final class KeySet<E> extends AbstractSet<E>implements NavigableSet<E>, Closeable, Serializable {
		...
		public NavigableSet<E> subSet(E fromElement, boolean fromInclusive, E toElement, boolean toInclusive) {
			return new KeySet<E>(m.subMap(fromElement, fromInclusive, toElement, toInclusive), hasValues);
		}
	...
	static public class SubMap<K, V> extends AbstractMap<K, V>implements ConcurrentNavigableMap<K, V> {

		public final BTreeMap<K, V> m;
		...
		@Override
		public Map.Entry<K, V> firstEntry() {
			Entry<K, V> k = lo == null ? m.firstEntry() : m.findLarger(lo, loInclusive);
		...
		@Override
		public Map.Entry<K, V> lastEntry() {
			Entry<K, V> k = hi == null ? m.lastEntry() : m.findSmaller(hi, hiInclusive);
	...
	static public class DescendingMap<K, V> extends AbstractMap<K, V>implements ConcurrentNavigableMap<K, V> {
		...
		private DescendingMap<K, V> newSubMap(K toKey, boolean toInclusive, K fromKey, boolean fromInclusive) {
			...
			return new DescendingMap<K, V>(m, fromKey, fromInclusive, toKey, toInclusive);
		}
	...
	public final Object modListenersLock = new Object();
	public Bind.MapListener<K, V>[] modListeners = new Bind.MapListener[0];

	@Override
	public void modificationListenerAdd(Bind.MapListener<K, V> listener) {
		synchronized (modListenersLock) {
			Bind.MapListener<K, V>[] modListeners2 = Arrays.copyOf(modListeners, modListeners.length + 1);
	...
	public void notify(K key, V oldValue, V newValue) {
	...
	void compactLevel(int level) {
		...
		long current = leftEdges.get(level + 1);
		long one = 0;
		while (current != 0) {
			lock(nodeLocks, current);
