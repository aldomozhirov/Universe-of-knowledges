...
import static com.database.uokdb.db.DataIO.*;
...
public class StoreWAL extends StoreCached {
	...
	public StoreWAL(String fileName) {
		this(fileName, fileName == null ? CC.DEFAULT_MEMORY_VOLUME_FACTORY : CC.DEFAULT_FILE_VOLUME_FACTORY, null,
				CC.DEFAULT_LOCK_SCALE, 0, false, false, null, false, false, false, null, null, 0L, 0L, false, 0L, 0);
	}

	public StoreWAL(String fileName, Volume.VolumeFactory volumeFactory, Cache cache, int lockScale,
			int lockingStrategy, boolean checksum, boolean compress, byte[] password, boolean readonly,
			boolean snapshotEnable, boolean fileLockDisable, HeartbeatFileLock fileLockHeartbeat,
			ScheduledExecutorService executor, long startSize, long sizeIncrement, boolean recidReuseDisable,
			long executorScheduledRate, int writeQueueSize) {
		super(fileName, volumeFactory, cache, lockScale, lockingStrategy, checksum, compress, password, readonly,
				snapshotEnable, fileLockDisable, fileLockHeartbeat, executor, startSize, sizeIncrement,
				recidReuseDisable, executorScheduledRate, writeQueueSize);
	...
	public void commit() {
		...
		try {
			...
			try {
				...
				longStackPagesLoop: for (int i = 0; i < set.length; i++) {
					long offset = set[i];
					if (offset == 0)
	...
	protected void replaySoft() {
		...
		for (int lockPos = 0; lockPos < locks.length; lockPos++) {
			...
			try {
				...
				indexValLoop: for (int pos = 0; pos < table.length;) {
				...
				dataLoop: for (int pos = 0; pos < table.length;) {
		...
		structuralLock.lock();
		try {
			dataLoop: for (int pos = 0; pos < committedPageLongStack.table.length;) {
		...
		if (CC.PARANOID) {
			...
			for (int i = 0; i < w.length - 1; i++) {
				...
				if (offset1 + size1 > offset2) {
					throw new AssertionError("write overlap conflict at: " + offset1 + " + " + size1 + " > " + offset2
							+ " (" + size2 + ")");
				}
			}
		}
	...
	protected boolean hasUncommitedData() {
		for (int i = 0; i < locks.length; i++) {
			...
			lock.lock();
			try {
				if (uncommittedIndexTable[i].size() != 0 || uncommittedDataLongs[i].size() != 0
						|| writeCache[i].size != 0)
					return true;
			} finally {
				lock.unlock();
